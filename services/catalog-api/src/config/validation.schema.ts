...
export const validationSchema = Joi.object({
  ...
  JWT_EXPIRES_IN: Joi.string().default('20m'),
  REFRESH_TOKEN_EXPIRES_DAYS: Joi.number().integer().min(1).max(90).default(30),
  REDIS_URL: Joi.string().allow('').default(''),
  TEST_AUTH_BYPASS_ENABLED: Joi.boolean().truthy('true').falsy('false').default(false),
  AI_INTEGRATION_ENABLED: Joi.boolean().truthy('true').falsy('false').default(true),
  ...
  try {
    assertProductionConfig({
      ...
      internalServiceToken: value.QALAGO_INTERNAL_SERVICE_TOKEN ?? '',
      aiIntegrationEnabled: value.AI_INTEGRATION_ENABLED !== false,
      testAuthBypassEnabled: value.TEST_AUTH_BYPASS_ENABLED === true,
